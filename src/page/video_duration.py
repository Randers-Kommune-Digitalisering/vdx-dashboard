...
from utils.vdx_data import get_vdx_data, retrieve_weekday_names, format_duration, get_month_names
...
def get_video_calls_duration():
    ...
    try:
        ...
        if content_tabs == 'Uge':
            ...
            with col1:
                if pd.isna(avg_duration_week):
                    avg_duration_week_display = "Ingen data"
                else:
                    avg_duration_week_display = format_duration(int(avg_duration_week))
                ui.metric_card(
            ...
            with col2:
                ...
                selected_month_tmp = st.selectbox(
                    ...
                    key=f'selected_month_{selected_year_month}',
            ...
            col1, col2 = st.columns([1, 2])
            with col1:
                if pd.isna(avg_duration_month):
                    avg_duration_month_display = "Ingen data"
                else:
                    avg_duration_month_display = format_duration(int(avg_duration_month))
                ui.metric_card(
